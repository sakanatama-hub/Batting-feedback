...
if not st.session_state["ok"]:
            ...
            if not pdf.empty:
                ...
                with c2:
                    ...
                    date_range = st.date_input(
                        "分析期間を選択",
                        value=(pdf['Date_Only'].min(), pdf['Date_Only'].max()),
                        key="analysis_range"
                    )
                
                # 期間が正しく選択されているか確認してフィルタリング
                ...
                if vdf.empty:
                    ...
                    if "スイング時間" in target_metric:
                        ...
                        c_scale, zm, zM, tv = [[0, "blue"], [0.5, "white"], [1, "red"]], 75, 135, [75, 105, 135]
                    ...
                    # --- 2. インパクトポイント ---
                    st.subheader(f"📍 {target_metric}：インパクトポイント全表示")
                    fig_point = go.Figure()
    ...
    with tab2:
        ...
        with st.expander("登録設定", expanded=True):
            ...
            with col1: reg_player = st.selectbox("登録する選手を選択", PLAYERS, key="reg_p_tab2")
            with col2: reg_date = st.date_input("登録する日付を選択", datetime.date.today(), key="reg_d_tab2")
