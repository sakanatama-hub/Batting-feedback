...
if not st.session_state["ok"]:
    ...
    with tab2:
        ...
        with st.expander("登録設定", expanded=True):
            ...
            with col1:
                reg_player = st.selectbox("登録する選手を選択", PLAYERS, key="reg_p_tab2")
            with col2:
                reg_date = st.date_input("登録する日付を選択", datetime.date.today(), key="reg_d_tab2")
            ...
            try:
                ...
                if st.button("GitHubへデータを保存"):
                    ...
                    if save_to_github(updated_db):
                        ...
                        st.balloons()
                        # 最新データを読み込み直す
                        st.session_state.db_df = load_data_from_github()
                    else:
                        st.error("❌ 保存に失敗しました。")
